...
def main_action(menu_action):
    while True:
        if menu_action == "NEW":
            addNewRecord(data)
        elif menu_action == "AVALUE":
            addValue(data)
        elif menu_action == "VIEW":
            viewRecord(data)
        elif menu_action == "AVERAGE":
            computeAverage(data)
        elif menu_action == "SAVE":
            saveFile(data)
        elif menu_action == "EXIT": # Add check if saved
            return "Goodbye"
        else:
...
def verify_id(data):
    while True:
        student_id_check = input("Please enter student ID or 'BACK' to go back: ").upper().strip()

        if student_id_check == "BACK": return "no data"

        if student_id_check in data: return student_id_check
        elif student_id_check not in data: return "new data"
        
        else: print("Invalid input, try again")


def new_user(id_data, data):
    while True:
        id_data = verify_id(data)

        if id_data == "no data": break

        if id_data == "new data":
            user_action = input(f"ID does not exist, create new user?\n"
                                f"YES/NO: ").upper().strip()
            if user_action == "NO":
                continue
            elif user_action == "YES":
                new_user(student_id_check)
            else:
                print("Invalid input, try again")
                continue
